...
export default function ItemsPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (editingId) {
      await fetch(apiUrl(`/api/items/${editingId}`), {
        method: "PUT",
    ...
    <main className="grid">
      <section className="card">
        ...
        <form onSubmit={handleSubmit} className="form">
          ...
          <input placeholder="Variants (comma separated)" value={form.variants} onChange={(e) => setForm({ ...form, variants: e.target.value })} />
          <input placeholder="Base Price" type="number" min="0" step="0.01" required value={form.basePrice} onChange={(e) => setForm({ ...form, basePrice: e.target.value })} />
